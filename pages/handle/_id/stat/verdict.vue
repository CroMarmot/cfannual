<template>
  <div class="problems">
    <div>
      <i18n path="verdict.acStatus">
        <template #submitCount>
          {{ userStatusResult(year).length }}
        </template>
        <template #acCount>
          {{ mergedUserStatusResult.filter((item)=>item.verdict === 'OK').length }}
        </template>
      </i18n>
    </div>
    <table>
